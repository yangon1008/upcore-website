...
const Hero: React.FC<HeroProps> = ({ onNavigate }) => {
  return (
    <section className="relative h-screen min-h-[900px] w-full bg-black flex items-center overflow-hidden">
      ...
      <div className="container mx-auto px-6 md:px-12 relative z-10 grid grid-cols-1 lg:grid-cols-12 items-center gap-0">
        ...
        <div className="lg:col-span-5 text-white space-y-10 order-2 lg:order-1 pt-12 lg:pt-0">
          ...
          <div className="flex items-center space-x-6">
            <button onClick={onNavigate} className="bg-white text-black px-10 py-4 rounded-full font-bold text-lg hover:bg-gray-200 transition-all shadow-xl hover:-translate-y-1">
              进一步了解
            </button>
            <button onClick={onNavigate} className="border border-white/20 text-white px-10 py-4 rounded-full font-bold text-lg hover:bg-white/5 transition-all backdrop-blur-sm">
              立即购买
            </button>
          </div>
        </div>
        ...
        <div className="lg:col-span-7 relative flex justify-center lg:justify-end items-center order-1 lg:order-2 h-full py-20 lg:py-0">
          <div className="relative group">
            ...
            <div onClick={onNavigate} className="absolute top-1/4 -right-12 bg-white/5 backdrop-blur-2xl p-6 rounded-[32px] border border-white/10 shadow-3xl hidden xl:block animate-bounce-slow cursor-pointer">
      ...
      {/* Navigation & Interaction UI */}
      <div className="absolute bottom-12 right-12 hidden md:flex items-center space-x-8 text-white/30 font-bold text-sm z-20">
        <button onClick={onNavigate} className="hover:text-white transition-colors flex items-center gap-2">
          <ChevronLeft size={20} /> PREV
        </button>
        <div className="h-[2px] w-24 bg-white/10 relative">
